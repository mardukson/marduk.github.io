			...
			switch ($TagType) {
				...
				case GETID3_FLV_TAG_META:
					if (!$found_meta) {
						...
						$copykeys = array('framerate'=>'frame_rate', 'width'=>'resolution_x', 'height'=>'resolution_y');
						foreach ($copykeys as $sourcekey => $destkey) {
							if (isset($ThisFileInfo['meta']['onMetaData'][$sourcekey])) {
								switch ($destkey) {
									case 'resolution_x':
									case 'resolution_y':
										$ThisFileInfo['video'][$destkey] = intval(round($ThisFileInfo['meta']['onMetaData'][$sourcekey]));
										break;
									case 'frame_rate':
									default:
										$ThisFileInfo['video'][$destkey] = $ThisFileInfo['meta']['onMetaData'][$sourcekey];
										break;
								}
							}
						}
