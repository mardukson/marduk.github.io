...
function loadIframe(ifr,url) { url=(url==null?"about:blank":url); if (ifr) document.frames(ifr.id).location.replace(url); return }
...
function IsInKC()
{
	...
	{
		if (typeof(window.external.provider)=="object") 
		{
			if (window.external.provider.name=="KeeBoo")
			{
				return true;
			}
		}
...
function initTitle()
{
	WL(titleStr(),"rightFolio")
	gui.titles.pgw=layerWidth("rightFolio")
	gui.titles.ph=layerHeight("rightFolio")	
...
function updtPages()
{
	var tr = resizeIframe(f_rightpage.id,wR,hp)
	if ( ! tr ) writePage('right');
	var tl = resizeIframe(f_leftpage.id,wL,hp)
	if ( ! tl ) writePage('left'); 
	updtLinks()
}
...
function gBook_write()
{
	if ( IsInKC() )
	{
		document.write("<frameset cols='*,0' frameborder='0'><frame name='f_import' src='"+_common+'import.htm'+"'></frameset>");
		document.close()
		return
	}
...
function gBook_load()
{
	...
	if (_ie)
		...
		}	gui.logo.w=layerWidth("logo"); gui.logo.h=layerHeight("logo")
...
{
	if ( init || IsInKC() ) return
	with(gui)
	{
		SLS( false, f_leftpage.id, mf_leftpage.id, f_rightpage.id, mf_rightpage.id );
		if (this.mode==2)
		{
			if ( !isAnimated || !left )
			{	
			...
			SLS( true, f_leftpage.id, mf_leftpage.id );
		}
		else
		...
		SLS( true, f_rightpage.id, mf_rightpage.id );
	}
	updtPages()
}
...
function loadCB()
{
	if ( IsInKC() )
	{
		parent.document.all.Process.style.left = -2*screen.availWidth
		parent.document.all.KeeBook.style.visibility= "visible"
		return;
	}
	
	_Book.load()
	parent.frames["Process"].preloadRefresh(true)
