...
function Toolbar_Source() 
{
	...
	s+=lyr(sw,sh,wriTxt(_lab["tools"],null,null,null,null,"toolbar"),"class=z100",_T,"btntoolbarH",mw,mh)
...
{
	var B=BT
	B.w=B.lvw+layerWidth("btntoolbarH")+B.rw
	WL("","tmp")
...
function IconState_Button()
{
	...
	with( this )
	{
		s+='<button style="position:absolute;background-color:transparent;border:none;cursor:hand;" id="stBut' + id + '"' 
		...
		s+='<td id="stButImg'+id+'" width="1">' + relImg(img,"butImg"+id,imgw,imgh,dx,dy) + '</td>'
		s+='<td class="toolbar" width="3">&nbsp;</td>'
		s+='<td class="toolbar" id="stButTxt'+id+'"><nobr>' + txt + '</nobr></td>'
...
function Icon_Load()
{
	with(this)
	{
		for ( var sid in States )
		{
			...
			st.txtw=layerWidth("stButTxt"+st.id)+7
...
function relImg(u,id,w,h,dx,dy,st,att)
{
	...
	return '<span '+(att?att:'')+' id="'+id+'" style="width:'+w+'px;height:'+h+'px;background-image:url('+u+');background-position:'+((-1)*dx)+' '+((-1)*dy)+';'+(st?st:"")+'"></span>';
}
