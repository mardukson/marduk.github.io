...
var _T=true,_F=false,_ww=0,_nav_wp=0,_nav_wn=0,_sl_gd=0,_sl_pas=0,_init=_T,_act=_F,_drag=_F
...
function load_sliderCB() 
{
	...
	for (var i=0; i<L; i++)	
	{
		...
		t.s=s
		WL(!(_ie4&&_mac)?"<nobr>"+t.s+"</nobr>":t.s,lid); t.w=layerWidth(lid);
	}
...
function WriteGrip(init) {
	if (init) 
		GP.w=GP.lw+layerWidth("gp_txt")+GP.rw
	RL("gripbg",GP.w-GP.lw-GP.rw,GP.h)
	WL('<a href="javascript:void(0)" onfocus="this.blur()" onmouseover="gripOver()" onmouseout="gripOut()" onmousedown="downCB(event)">'+box(GP.w,GP.h,' ondragstart="return _F;"',_hand)+'</a>',"gp_lnk")
	RL("gp_txt",GP.w,GP.h)
...
function setTextInfo(text,force) 
{
	...
	text='<span class="info"><nobr>'+text+'</nobr></span>'

	var lid=(_ie4&&!_mac?"ie4win":"truc")
	WL(text,lid)
	var txt_width=layerWidth(lid)
...
function moveCB(ev) 
{
	...
	{
		SetPageInfo(_T)
		INTER_UpdateSlider(_Book.page,ev)
		return _T
	}
	if (_drag) 
	{
...
/*
		...
		if ((x<_gp_X)&&(x>_nav_wp)&&(y>=_gp_Y)&&(y<=_gp_Y+GP.h)) setTextInfo((fl?_lab.previousPagesH:_lab.previousPageH)+" "+_lab.accPrevPage)
		if ((x>_gp_X+GP.w)&&(x<_ww-_nav_wn)&&(y>=_gp_Y)&&(y<=_gp_Y+GP.h)) setTextInfo((fl?_lab.nextPagesH:_lab.nextPageH)+" "+_lab.accNextPage)
	}
*/
	return _T
}
function upCB(ev) 
{
	...
	document.mousemove = new Function("return false;")
	document.mouseup = new Function("return false;")
	var ope 
	if (!_drag) {
	...
	changePage(_sl_pg,null,null,null,ope,"nav")
	_drag=_F
	_act=_F
	SetPageInfo(_T)
	return _F
...
function updtNavButton(side,notext,init)
{
	...
	if (isout) { off=sw; offout=bmargin; } else { off=bmargin; offout=sw; }
...
{ 
	gui.prev.w=layerWidth("prevButton");gui.next.w=layerWidth("nextButton");
	var maxw=Math.max(gui.prev.w,gui.next.w);
	gui.prev.txtl=maxw-gui.prev.w; gui.next.txtl=maxw-gui.next.w
