...
function write_slider() 
{
	...
	lyr(sw,sh, SK("notile.gif","gripl",0,0,GP.lw,GP.h,GP.dx,0)+
		SK("htile.gif","gripbg",GP.lw,0,GP.w,GP.h,0,GP.dy)+
		SK("notile.gif","gripr",GP.lw+GP.w,0,GP.rw,GP.h,GP.dx+GP.lw,0) ,'class=grip',_F,"grip")+

	lyr(l,0,"",'class=grip',_F,"gp_txt")+
...
function SetPageInfoDoit(clear) 
{
	...
	if (_Book.mode==1) 
	{
		setTextInfo(_sl_pgs[_sl_pg].s,_F); 
		return;
	}

	var tl = _sl_pgs[_sl_pg]
	var tr = _sl_pgs[_sl_pg+1]
	
	window.status = tl.s + " | " + tr.s
	return;
}
...
function WriteGrip(init) {
	if (init) 
	{
		//GP.w=GetTextWidth("&nbsp;"+(book.pages.length-1)+"&nbsp;-&nbsp;"+book.pages.length+"&nbsp;","grip")
		GP.w=GP.lw+GP.w+GP.rw
	}
	//RL("gripbg",GP.w-GP.lw-GP.rw,GP.h)
	WL('<a href="javascript:void(0)" onfocus="this.blur()" onmouseover="gripOver()" onmouseout="gripOut()" onmouseup="upCB(event)" onmousedown="downCB(event)" ondragstart="return false;">'+box(GP.w,GP.h,null,_hand)+'</a>',"gp_lnk")
	RL("gp_txt",GP.w,GP.h)
...
{ 
	gui.prev.w=GetTextWidth(_lab["previousPage"+(fl?"s":"")],"nextprev")+35; 
	gui.next.w=GetTextWidth(_lab["nextPage"+(fl?"s":"")],"nextprev")+35;
...
function elemTab()
{
	...
	with(this)
	{
		...
		s+=lyr(sw,sh,inner((text!=null?text:""),false),"",false,id,minw,minh)
