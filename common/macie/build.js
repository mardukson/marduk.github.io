...
function GetTextWidth( txt, cls )
{
	...
	var el = layerElem("tmpLayer", parent )
	el.innerHTML = '<span '+cls+'>'+txt+'</span>'
...
function gBook_write()
{
	...
	with(gui)
	{
		...
		SK("htile.gif","tTBg",sw,sh,0,bgceil.h,0,bgceil.dy)+
		...
		lyr(sw,sh,"<span class='theLogo'><nobr>&nbsp;KeeBook &#8482;</nobr></span>","",true,"logo",mw,mh)+
...
function gBook_resizeBook(init)
{
	...
	with(gui)
	{
		...
		ML("tTBg",x0t+bgceil.lw,y1-ceil.h+bgceil.t-3)
		...
		RL("colorleft",fill.lcb+w1+fill.mlcb,h1+fill.ch+fill.bh)
		ML("colorleft",x2-fill.lcb,y1-fill.ch)
		RL("patternleft",fill.lcb+w1+fill.mlcb,h1+fill.ch+fill.bh)
		ML("patternleft",_Book.mode==2?x2-fill.lcb:sw,_Book.mode==2?y1-fill.ch:sh)
		...
		RL("colorright",fill.mrcb+w2+fill.rcb,h1+fill.ch+fill.bh)
		ML("colorright",x4-fill.mrcb,y1-fill.ch)
		RL("patternright",fill.mrcb+w2+fill.rcb,h1+fill.ch+fill.bh)
		ML("patternright",x4-fill.mrcb,y1-fill.ch)	
...
function gBook_resizePages(init)
{
	...
	with(gui)
	{
		if (this.mode==2)
		{
			if ( !isAnimated || !left )
			{	
				RL(f_leftpage.id,w1,hp)
				RL(mf_leftpage.id,w1,hp)
				ML(f_leftpage.id,x2,yp)
				ML(mf_leftpage.id,sw,sh)
			}
			RL("leftTitle",w1-2*titles.margin,titles.h)
			ML("leftTitle",x2+titles.margin,yt)
			ML("leftFolio",x3-(titles.margin+titles.pgw),yt)
		}
		else
		...
		if ( !isAnimated || !right )
		{
			RL(f_rightpage.id,w2,hp)
			RL(mf_rightpage.id,w2,hp)
			ML(f_rightpage.id,x4,yp)
			ML(mf_rightpage.id,sw,sh)
		}
		RL("rightTitle",w2-2*titles.margin,titles.h)
		ML("rightTitle",x4+titles.margin,yt)
		ML("rightFolio",x5-(titles.margin+titles.pgw),yt)
	}
}
