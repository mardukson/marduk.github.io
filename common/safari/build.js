...
function gBook_init()
{
	...
	_localized=calPath(_root,EnginePath+params.language);
	_inc=calPath(_root,(_dyn?"../../":"")+"include/")
	_resources=calPath(_common,"../resources/")
	_neu=calPath(_root,EnginePath+params.skin);
	...
	with(gui)
	{
		if (this.mode==2)
		{
			...
			{	
				...
				ML(f_leftpage.id,xL,yp)
				ML(mf_leftpage.id,sw,sh)
			}
			RL("leftTitle",wL-2*titles.margin,titles.h)
			ML("leftTitle",xL+titles.margin,yt)
			ML("leftFolio",xL+wL-(titles.margin+titles.pgw),yt)
		}
		else
		...
		ML("rightTitle",xR+titles.margin,yt)
		ML("rightFolio",xR+wR-(titles.margin+titles.pgw),yt)
	}
}
