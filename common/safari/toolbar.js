...
function WriteToggle() 
{
	...
	B.txtl=Math.round((B.w-GetTextWidth(_lab["tools"],"toolbar"))/2)
	WL("","tmp")
	...
	WL(skinImg("notile.gif","bl",0,0,lw,B.h,ldx,0)+
		...
		wriTxt(_lab["tools"],"toolstext",null,B.txtl,B.txtt,"toolbar")
	,"btntoolbarH")	
...
function IconState_Button()
{
	...
	with( this )
	{
		s+= '<span id="stBut' + id + '" style="position:absolute;background-color:transparent;border:none;">'
		s+='<table cellpadding="0" cellspacing="0" border="0"><tr valign="middle">'
		s+='<td>' 
		s+='<a onmouseover="ButtonMouseOver(\'' + iconid + '\')"'
			+ ' onmouseout="ButtonMouseOut(\'' + iconid + '\')"'
			+ ' onclick="ButtonClick(\'' + iconid + '\')"'
			+ ' onfocus="this.blur()"'
			+ '>' 
		s+=relImg(img,"stButImg"+id,imgw,imgh,dx,dy) + '</a></td>'
		s+='<td class="toolbar" width="3">&nbsp;</td>'
		s+='<td class="toolbar"><span id="stButTxt'+id+'" style="position:relative;"><nobr>' 
		s+='<a onmouseover="ButtonMouseOver(\'' + iconid + '\')"'
			+ ' onmouseout="ButtonMouseOut(\'' + iconid + '\')"'
			+ ' onclick="ButtonClick(\'' + iconid + '\')"'
			+ ' onfocus="this.blur()"'
			+ '><u>'
		s+= txt + '</u></a></nobr></span></td>'
