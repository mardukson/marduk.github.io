...
function write_slider() 
{
	...
	SK("htile.gif","gripbg",sw,sh,0,GP.h,0,GP.dy)+
	SK("notile.gif","gripr",sw,sh,GP.rw,GP.h,GP.dx+GP.lw,0)+

	lyr(0,0,(!_ns4?"<nobr>&nbsp;"+(book.pages.length-1)+"&nbsp;-&nbsp;"+book.pages.length+"&nbsp;</nobr>":''),'class=grip',_F,"gp_txt",mw,mh)+
	...
	if (_ie4&&!_mac) {
		s+='<span class="info" id="ie4win" style="position:relative; visibility:hidden;"></span>'
	}
...
function load_sliderCB() 
{
	...
	if (_ns4) 
	{
		...
		var w=L*GP.w
		var str=S+' width='+w+'><tr>'
		for (var i=0; i<L; i++)
			str+='<td align=center width="' + GP.w + '" align="center" class="grip">'+(i+1)+'</td>'
		str+='</tr></table>'
		WL(str,"gp_txt1")	
		...
		var str=S+' width='+w+'><tr>'
		for (var i=0; i<L; i++)
			str+='<td align=center width="' + GP.w + '" align="center" class="grip">'+(i+1)+'&nbsp;-&nbsp;'+(i+2)+'</td>'
		str+='</tr></table>'
		WL(str,"gp_txt2")	

		var tl=new Array,tr=new Array()
		tl[tl.length]=S+' width='+L*infoWidth+'><tr>'
		tr[tr.length]=S+' width='+L*infoWidth+'><tr>'
		for (var i=0; i<L; i++) {
			var inf=getPageTitle(i,_T)	
			tabInfo[i]=new Object
			tabInfo[i].text=inf
			tabInfo[i].w=infoWidth
			tabInfo[i].offx=i*infoWidth
			tl[tl.length]='<td align="right" width="'+infoWidth+'" class="info">'+tabInfo[i].text+'</td>'
			tr[tr.length]='<td align="left" width="'+infoWidth+'" class="info">'+tabInfo[i].text+'</td>'
		}
		tl[tl.length]='</tr></table>'
		tr[tr.length]='</tr></table>'
		WL(tl.join(''),"h_l");	
		WL(tr.join(''),"h_r");
...
function resize_sliderCB(init) 
{
	...
	if (_ww-(_nav_wp+_nav_wn)-(BS.lw+BS.rw)>GP.w) 
	{
		...
		{
			WL("|", "ie4win")
			pp_w=layerWidth("ie4win")
		}

		susp_w=layerWidth("h_susp")
		if (_ie4&&!_mac) {
			WL("...", "ie4win")
			susp_w=layerWidth("ie4win")
		}
...
function SetGripInfo() 
{
	...
	if (_ie) {
		S=""+(_sl_pg+1)
		if (_Book.mode==2) S+=" - "+(_sl_pg+2);
		WL(S,"gp_txt",true)
		if (_macIE)
		{
			WL(S,"truc",true)
			ML("gp_txt", _gp_X + (GP.w/2) - (layerWidth("truc")/2))
		}
		return
	}
	if (_moz) {
	...
	var x1,x2,x3,id
	SLS(_F,"gp_txt1","gp_txt2")
	x1=_sl_pg*GP.w
	x2=x1+GP.w
	id="gp_txt"+_Book.mode
	SCR(layerCSS(id),x1,0,x2,GP.h)	
	ML(id,_gp_X-x1,_gp_Y+GP.txtt)
	SL(_T,id)
	return
}
...
function SetPageInfoDoit(clear) 
{
	...
	if (_ie||_moz) 
	{
		...
		if ( t )
		{
			WL(!(_ie4&&_mac)?"<nobr>"+t.s+"</nobr>":t.s,"h_l")
			l_w=t.w
	
			t=_sl_pgs[_sl_pg+1]
	
			WL(!(_ie4&&_mac)?"<nobr>"+t.s+"</nobr>":t.s,"h_r")
			r_w=t.w
	...
	else 
	{
		with(tabInfo[_sl_pg]) {
			l_w=w
			SCR(layerCSS("h_l"),offx+l_w-av_w,0,offx+l_w,20)
			ML("h_l",BI.l+av_w-offx-w-pp_w,y2+BI.t+BI.txtt)
		}
		with(tabInfo[_sl_pg+1]) {
			r_w=w	
			SCR(layerCSS("h_r"),offx,0,offx+av_w,20)
			ML("h_r",BI.l+av_w-offx+pp_w,y2+BI.t+BI.txtt)
		}
		SLS(_T,"h_l","h_r","h_pipe")
	}
}
function WriteGrip(init) {
	...
	RL("gripbg",GP.w-GP.lw-GP.rw,GP.h)
	WL('<a href="javascript:void(0)" onfocus="this.blur()" onmousedown="downCB(event)" ondragstart="return false;">'+box(GP.w,GP.h,null,_hand)+'</a>',"gp_lnk")
	RL("gp_txt",GP.w,GP.h)
...
{
	ML("gripl",l,t); ML("gripbg",l+GP.lw,t); ML("gripr",l+GP.w-GP.rw,t);
	ML("gp_lnk",l,t)
	if (_macIE) { ML("gp_txt", _gp_X + (GP.w/2) - (layerWidth("truc")/2),t+GP.txtt); return; }
	if(_ie||_moz) ML("gp_txt",l+(_macIE?gui.grip.lw:0),t+GP.txtt)
}
...
var ns_return = _T
	...
	if (_ns4&&_box_btn) ns_return = BtnAction(_box_btn, "DOWN")
	...
	return (_moz?_F:_ns4?ns_return:_T)
}
...
function moveCB(ev) {
	...
	if (_drag) {
		...
		if ((x<_gp_X)&&(x>_nav_w)&&(y>=_gp_Y)&&(y<=_gp_Y+GP.h)) setTextInfo((fl?_lab.previousPagesH:_lab.previousPageH)+(!_ns4?" "+_lab.accPrevPage:""))
		if ((x>_gp_X+GP.w)&&(x<_ww-_nav_w)&&(y>=_gp_Y)&&(y<=_gp_Y+GP.h)) setTextInfo((fl?_lab.nextPagesH:_lab.nextPageH)+(!_ns4?" "+_lab.accNextPage:""))
	}
	return _T
}
function upCB(ev) 
...
function trLnkMouseOver(target) 
{
	if (_drag) return _F
	switch(target) {
		case 'prev': setTextInfo((fl?_lab.previousPagesH:_lab.previousPageH)+(!_ns4?" "+_lab.accPrevPage:"")); break
		case 'next': setTextInfo((fl?_lab.nextPagesH:_lab.nextPageH)+(!_ns4?" "+_lab.accNextPage:"")); break
		case 'toggle': setTextInfo((gui.toolbar.show?_lab.hideToolbarH:_lab.showToolbarH)+(!_ns4?" "+_lab.accToolbar:""));break
	}
	return _T
}
...
{ 
	gui.prev.w=GetTextWidth(_lab["previousPage"+(fl?"s":"")],"nextprev")+10; gui.next.w=GetTextWidth(_lab["nextPage"+(fl?"s":"")],"nextprev")+10;
...
function wriTxt(txt,id,mid,l,t,cl)
{
	...
	if ( rel ) 
		...
		if(_ie&&mid) txt="<SPAN class="+cl+" id=theLink>"+txt+"</SPAN>"
	else s=lyr(l,t,'<nobr>&nbsp;'+txt+'&nbsp;</nobr>'," class="+cl,true,id,mw,mh)
	return s 
}
...
function overTabCB(n,ev)
{
	...
	if (tab.w!=tab.vw+TS.w3-TS.lw-TS.rw)
	{
		...
		else s="<table class=toolTipBorder border=0 cellpadding="+(_ie4?2:0)+" cellspacing=0><tr><td><span class=toolTipBorder><nobr>%1</nobr></span></td></tr></table>"
