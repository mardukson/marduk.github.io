...
function IconState_Button()
{
	...
	with( this )
	{
		s+='<button style="position:absolute;background-color:transparent;border:none;cursor:hand;" id="stBut' + id + '"' 
			...
			+ ' onfocus="this.blur()"'
			+ '>'
		s+='<table cellpadding="0" cellspacing="0"><tr valign="middle">'
		s+='<td>' + relImg(img,"stButImg"+id,imgw,imgh,dx,dy) + '</td>'
		s+='<td class="toolbar" width="3">&nbsp;</td>'
		s+='<td><span id="stButTxt'+id+'" style="position:relative;" class="toolbar"><nobr>' + txt + '</nobr></span></td>'
...
function Icon_Move(l,t,save)
{
	...
	with(this)
	{
		...
		var el = layerElem("stButImg"+st.id)
	}
